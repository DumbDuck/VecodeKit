...
VKQuartzPicture g_picture_all_test = {
    { 0, 0, 550, 400 },
    draw_main,
};
...
static void draw_gradient_1(CGContextRef ctx)
{
    ...
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGGradientRef gradient = CGGradientCreateWithColorComponents(colorSpace, colors, locations, 7);
    CGColorSpaceRelease(colorSpace);
    CGGradientDrawingOptions options = kCGGradientDrawsBeforeStartLocation | kCGGradientDrawsAfterEndLocation;
...
static void TKColorApplyColorTransform(CGFloat color[4], const TKColorTransform* transform)
{
    if (transform)
    {
        const CGFloat* mult = transform->mult;
        const CGFloat* add = transform->add;
        color[0] = mult[0] * color[0] + add[0];
        color[1] = mult[1] * color[1] + add[1];
        color[2] = mult[2] * color[2] + add[2];
        color[3] = mult[3] * color[3] + add[3];
    }
}

static void draw_gradient_2(CGContextRef ctx, const TKColorTransform* colorTrans)
{
    CGFloat colors[] = {
        1, 0, 0, 1,
        1, 1, 0, 1,
...
static void TKSetRGBStrokeColor(CGContextRef ctx, CGFloat r, CGFloat g, CGFloat b, CGFloat a, const TKColorTransform* transform)
{
    CGFloat color[4] = { r, g, b, a };
    TKColorApplyColorTransform(color, transform);
    CGContextSetRGBStrokeColor(ctx, color[0], color[1], color[2], color[3]);
}

static void draw_shape_2(CGContextRef ctx, const TKColorTransform* colorTrans)
{
...
static void draw_gradient_3(CGContextRef ctx, const TKColorTransform* colorTrans)
{
    CGFloat colors[] = {
        0, 1, 0, 1,
        0, 0, 0, 1
...
static void draw_shape_4(CGContextRef ctx, const TKColorTransform* colorTrans)
{
    CGContextSaveGState(ctx);
    CGContextConcatCTM(ctx, CGAffineTransformMake(1, 0, 0, 1, 0, 0));
    def_path_6(ctx);
